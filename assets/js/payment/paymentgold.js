...
function saveSalesPaymentGoldVenkat() {
    var completeurl = url + "payment-payment/makeSalesPaymentGoldVenkat";
    var place = "salesPaymentPopupWS";
    var paymentDate = $("#paymentDate").val();
    var paymentBank = $("#paymentBank").val();
    var paymentMode = $("#paymentMode").val();
...
function printPendingReceipt(company)
{
    var completeurl = url + 'payment-payment/exportPrintPendingReceipt?company=' + company;
    window.open(completeurl);
}
function completedSalesPaymentDetailsGold(salesBillId)
{
    var requesturl = url + 'payment-payment/completedSalesPaymentDetailsGold';
    var data = "salesBillId=" + salesBillId;
    $("#loadSalesPaymentWithinStateDetails").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadSalesPaymentWithinStateDetails');
}