...
function addNewProduct() {
    ...
    $("#salesproductName").val(0).trigger("change");
...
function billItemSave() {
    ...
    var igstRate = $("#igstRate").val();
    var packingFactor = $("#packingFactor").val();
    var totalRate = parseFloat(cgstRate) + parseFloat(sgstRate) + parseFloat(igstRate);
//    alert(cgstRate + " " + sgstRate + " " + igstRate);

 var baseamount =  $("#unitRate").val();
    if ($("#taxType").val() == "1") {
        var totalRate = parseFloat(cgstRate) + parseFloat(sgstRate) + parseFloat(igstRate);
        var divideFactor = 100 + totalRate;
    }

    if ($("#unitRateType").val() == "1") {
        var unitRateOrginal = $("#unitRate").val() * parseFloat(packingFactor);
//alert($("#unitRateType").val());
//alert(unitRateOrginal);

    } else
    {
        var unitRateOrginal = (parseFloat(baseamount) * 100) / (100 + parseFloat(totalRate));
    ...
    if ($("#taxType").val() == "1") {
        ...
        var unitRate1 = parseFloat(unitRateOrginal) * parseFloat(billFactor) * 100 / divideFactor;
        var unitRate = unitRate1.toFixed(2);
    } else
    {
        var unitRate =Math.round(parseFloat(unitRateOrginal) * parseFloat(billFactor)* 100)/ 100;
    ...
    var lineproductdescription = $("#description").val();
    var billitemrow = '<div class="input-field col s12" id="billItemRow' + rowcount + '">\n\
                        ...
                        <input readonly  name="lineproductName[]" type="hidden" value="' + productName + '">\n\
                         \n\<label>' + productName + '</label>\n\
                         </div>\n\
                        \n\<div class="input-field col s2">\n\
                        <input readonly  name="lineproductdescription[]" type="hidden" value="' + lineproductdescription + '">\n\
                       \n\<label>' + lineproductdescription + '</label>\n\
                         </div>\n\
                        <div class="input-field col s1">\n\
                      ...
                      <input readonly  name="linepackingfactor[]" type="hidden" value="' + packingFactor + '">\n\
                        ...
                        <div class="input-field col s1">\n\
                         <input readonly  name="linetotal[]" type="hidden" value="' + total + '">\n\
                         <input readonly  name="linetotalwithtax[]" type="text" value="' + totalwithTax + '">\n\
                        </div>\n\
                        <div class="input-field col s1" style="margin-top:25px; margin-left:10px;">\n\
...
function loadPayrollDetails()
{
    ...
    var requesturl = url + 'sales-salesmalleswara/loadPayrollDetail';
    var data = "&fromDate=" + fromDate + "&toDate=" + toDate + "&voucherNo=" + voucherNo;

    $("#loadSearchPayrollGrid").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadSearchPayrollGrid');
}

function loadPayrollDataById(payrollId) {
    var requesturl = url + 'sales-salesmalleswara/loadPayrollDetailById';
    var data = "&payrollId=" + payrollId;
...
function deletePayrollEntry(payrollId , expenseId ) {
    $('#PayrollEntryDeletePopup').openModal({dismissible: false});
    $("#payrollId").val(payrollId);
    $("#expenseId").val(expenseId);
}

function deletePayrollEntries() {
    var completeurl = url + "sales-salesmalleswara/deletePayrollEntry";
    var place = "PayrollEntryDeletePopup";
...
function calculateTotalValue() {
    ...
    var igstValue1 = 0.00;
    var freight;
    var freightvalue = 0.00;
    var igstfreightvalue = 0.00;
    var packingexpense = $("#packingexpense").val();
    var less = $("#less").val();
    for (increment = 0; increment < linetotal.length; increment++) {
        subtotal = subtotal + parseFloat(linetotal[increment]);
        cgstValue1 = cgstValue1 + (parseFloat(linecgstrate[increment]) *
         parseFloat(linetotal[increment]) / 100);
         sgstValue1 = sgstValue1 + (parseFloat(linesgstrate[increment]) *
         parseFloat(linetotal[increment]) / 100);
         igstValue1 = igstValue1 + (parseFloat(lineigstrate[increment]) *
         parseFloat(linetotal[increment]) / 100);
        totalbags = totalbags + parseFloat(linetotalbags[increment]);

    }
    var lessSubtotal = parseFloat(subtotal) - parseFloat(less);
//    cgstValue1 = cgstValue1 + (parseFloat(linecgstrate) *
//            parseFloat(lessSubtotal) / 100);
//    sgstValue1 = sgstValue1 + (parseFloat(linesgstrate) *
//            parseFloat(lessSubtotal) / 100);
//    igstValue1 = igstValue1 + (parseFloat(lineigstrate) *
//            parseFloat(lessSubtotal) / 100);
//     alert(linecgstrate + " " + linesgstrate + " " + lineigstrate);
console.log("cgstValue1" . cgstValue1);
console.log("cgstValue1" . cgstValue1);
    $("#packingexpense").val(lessSubtotal.toFixed(2));
    /*freight = parseFloat(packingexpense) * 5 / 100;
    freightvalue = parseFloat(freight) / 2;
    igstfreightvalue = parseFloat(freight);
    var cgstValue = parseFloat(cgstValue1.toFixed(2)) + parseFloat(freightvalue);
    var sgstValue = parseFloat(sgstValue1.toFixed(2)) + parseFloat(freightvalue);
    var igstValue = parseFloat(igstValue1.toFixed(2)) + parseFloat(igstfreightvalue);
    if ($("#billGSTType").val() == "1") {
        var grandTotal = parseFloat(cgstValue.toFixed(2)) + parseFloat(sgstValue.toFixed(2)) + parseFloat(lessSubtotal) + parseFloat(packingexpense);
    } else {
        var grandTotal = parseFloat(igstValue.toFixed(2)) + parseFloat(lessSubtotal) + parseFloat(packingexpense);
    }*/
    var cgstValue = parseFloat(cgstValue1.toFixed(2)) + 0;
    var sgstValue = parseFloat(sgstValue1.toFixed(2)) + 0;
    var igstValue = parseFloat(igstValue1.toFixed(2)) + 0;
    if ($("#billGSTType").val() == "1") {
        var grandTotal = parseFloat(cgstValue.toFixed(2)) + parseFloat(sgstValue.toFixed(2)) + parseFloat(lessSubtotal);
    } else {
        var grandTotal = parseFloat(igstValue.toFixed(2)) + parseFloat(lessSubtotal);
    }
    ...
    var posting = $.post(completeurl,
            {
                ...
                lrno: $("#lrno").val(),
                lrdate: $("#lrdate").val(),
                marksonpackage: $("#marksonpackage").val(),
                packingexpense: $("#packingexpense").val(),
                less: $("#less").val()
            });
    posting.done(function (data) {
        $("#" + place).html(data);
...
function makePurchaseInvoice() {
    ...
    if (parseFloat($("#grandTotal").val()) > 10000 && ((($("#billType").val()) == 2) || ($("#billType").val()) == 3)) {
        ...
        var place = "mainModal";
        var customerSiteName = $("#customerSiteName").val();
        alert("custiemmeritm"+customerSiteName);
        exit()
        $("#" + place).html("Loading");
