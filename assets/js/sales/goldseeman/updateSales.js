...
function printAction(gstType, company, accountYear)
{
    ...
    var completeurl = url + 'sales-sales/printPdf?company=' + company + '&accountYear=' + accountYear +
            '&frombillnumber=' + frombillnumber + '&tobillnumber=' + tobillnumber + '&gstType=' + gstType + '&billType=' + billType;
...
function loadEstimateDetails(vatCstFlag)
{
    var requesturl;
    if (vatCstFlag == 0) {
        requesturl = url + 'sales-salesmalleswara/loadEstimateDetails';
    } else {
        requesturl = url + 'sales-salesmalleswara/loadSalesDetailsCstVat';
    }
    var data = "";
...
function loadEstimateGridDetails()
{
    var fromDate = $("#fromDate").val();
    var toDate = $("#toDate").val();
    var requesturl = url + 'sales-salesmalleswara/loadEstimateGridDetails';
    var data = "&fromDate=" + fromDate + "&toDate=" + toDate;

    $("#loadEstimateGrid").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadEstimateGrid');
}
function printEstimateBill(salesbillid, company, accountYear)
{
    var billType = 3;
    var gstType = 3;
    //var tobill = $("#toBill").val();
    /*var completeurl = url + 'sales-salesmalleswara/generateInvoicePdf?company=' + company + '&accountYear=' + accountYear +
            '&frombillnumber=' + frombillnumber + '&tobillnumber=' + tobillnumber + '&gstType=' + gstType + '&billType=' + billType;*/
    var completeurl = url + 'sales-sales/estimatePrintPdf?company=' + company + '&accountYear=' + accountYear +
            '&salesbillid=' + salesbillid + '&billType=' + billType + '&gstType=' + gstType;
    window.open(completeurl);
}
