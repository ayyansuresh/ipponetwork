    ...
    if (productId == "") {
        ...
        for (var i = 0; i < loadedItemName.item.length; i++) {
            if (loadedItemName.item[i].ItemId === productId) {
                ...
                $("#itemTypeId").val(loadedItemName.item[i].itemTypeId);
...
function tagBillItemSave() {
    ...
    var customerPrefix = $("#purchaseCustomer").val();
    var dateArray = customerPrefix.split("-");
    var customerPrefixId = dateArray[0];
    var $customerId = dateArray[1];
    var TagId = $("#lastTagId").val();
    //var salesbillprefix = $("#salesbillprefix").val();
    var salesbillprefix = customerPrefixId;
    ...
    var billitemrow = '<div class="input-field col s12" id="billItemRow' + rowcount + '">\n\
                        ...
                        <div class="input-field col s1">\n\
                        <input readonly  name="lineItemTypeId[]" type="hidden" value="' + itemTypeId + '">\n\ \n\
                        <input readonly  name="hsnCode[]" type="text" value="' + hsnCode + '">\n\
                        </div>\n\
                        <div class="input-field col s1" ' + cgstdisplay + '>\n\
                        <input readonly  name="linecgstRate[]" type="text" value="' + cgstRate + '">\n\
                      ...
                      <input readonly  name="linepackingfactor[]" type="hidden" value="' + packingFactor + '">\n\
                        ...
                        <div class="input-field col s3">\n\
                         ...
                         <input readonly name="lineCustomerId[]" type="hidden" value="' + $customerId + '">\n\
...
function makeTagEntery() {
    ...
    var lineItemTypeId = $('input[name="lineItemTypeId[]"]').map(function() {
        return $(this).val();
    }).get();
    var linecommodityRefId = $('input[name="linecommodityRefId[]"]').map(function() {
        return $(this).val();
    ...
    var lineCustomerId = $('input[name="lineCustomerId[]"]').map(function() {
        return $(this).val();
    }).get();
...
function loadTagEntryUpdateDetails()
{
    tagNumber = $("#tagNumber").val();
    //toDate = $("#toDate").val();
    var requesturl = url + 'sales-salesmalleswara/loadTagEntryUpdateDetail';
    var data = "&tagNumber=" + tagNumber;
...
function setTagEntryUpdate(updateTagIdNew, fromDateNew, toDateNew)
{
    var fromDate = fromDateNew;
    var toDate = toDateNew;
    var updateTagId = updateTagIdNew;
    var tagUpdateGram = $("#tagUpdateGram").val();
    var wastage = $("#wastage").val();
    var makingCharge = $("#makingCharge").val();
    $('#mainModal').openModal({dismissible: false});
    var completeurl = url + 'sales-salesmalleswara/setTagEnteryUpdate';
    var place = "mainModal";
    $("#" + place).html("Loading");
    var posting = $.get(completeurl,
            {
...
function getWastageDetail(itemGram, productName)
{
    var requesturl = url + 'sales-salesmalleswara/getWastageDetail';
    var salesproductName = productName;
    var data = "salesproductName=" + salesproductName + "&itemGram=" + itemGram;
    $("#loadWastageRate").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadWastageRate');
}
function setTagEntryDelete(updateTagIdNew)
{
    var updateTagId = updateTagIdNew;
    var tagUpdateGram = $("#tagUpdateGram").val();
    var wastage = $("#wastage").val();
    var makingCharge = $("#makingCharge").val();
    $('#mainModal').openModal({dismissible: false});
    var completeurl = url + 'sales-salesmalleswara/setTagEntryDelete';
    var place = "mainModal";
    $("#" + place).html("Loading");
    var posting = $.get(completeurl,
            {
