function loadBillDetails(vatCstFlag)
{
    ...
    if (vatCstFlag == 0) {
        ...
        {
            requesturl = url + 'sales-salesmalleswara/loadSalesDetailsRetail';
        }
    } else {
...
function printActionThermal(gstType, company, accountYear)
{
    var frombillnumber = $("#frombillnumber").val();
    var tobillnumber = $("#tobillnumber").val();
    var billType = 1;
    //var tobill = $("#toBill").val();
    var completeurl = url + 'sales-salesmalleswara/generateprintthermal?company=' + company + '&accountYear=' + accountYear +
            '&frombillnumber=' + frombillnumber + '&tobillnumber=' + tobillnumber + '&gstType=' + gstType + '&billType=' + billType;
    window.open(completeurl);
}