...
function addNewProduct() {
    ...
    var select2 = $('#commodityRefId').data('select2');
    $("#commodityRefId").val('').trigger("change");
    select2.open();

    $("#commodityRefId").val(null).trigger("change");
    $("#vad").val("");
    $("#makingCharge").val("");
    $("#productquantity").val("");
    $("#unitRate").val("");
...
function calculateTotalValue() {
   ...
   console.log(lineproductName);
    ...
    for (increment = 0; increment < linetotal.length; increment++) {
        if (lineproductId[increment] != "") {
            purchasetotal = purchasetotal + parseFloat(linetotal[increment]);
            /*cgstValue = cgstValue + (parseFloat(linecgstRate[increment]) *
             parseFloat(linetotal[increment]) / 100);
             sgstValue = sgstValue + (parseFloat(linesgstRate[increment]) *
             parseFloat(linetotal[increment]) / 100);
    ...
    if (checkBox.checked == true) {
        cgstValue = gstSubtotal * cgstRate / 100;
        sgstValue = gstSubtotal * sgstRate / 100;
    } else {
        cgstValue = 0.00;
        sgstValue = 0.00;
    }
    var paidAmount = $("#paidAmount").val();
    var gstSubtotalFinal = gstSubtotal;
    var grandTotal1 = parseFloat(cgstValue.toFixed(2)) + parseFloat(sgstValue.toFixed(2)) + parseFloat(gstSubtotalFinal.toFixed(2));
    var grandTotal = grandTotal1 - paidAmount;
...
function makeSalesInvoice() {
    ...
    var linehsnCode = $('input[name="linehsnCode[]"]').map(function() {
        return $(this).val();
    }).get();
    console.log(linehsnCode);
    var linecommodityRefId = $('input[name="linecommodityRefId[]"]').map(function() {
        return $(this).val();
    }).get();

    var linecommodityName = $('input[name="linecommodityName[]"]').map(function() {
        return $(this).val();
    }).get();
    var discount = $('#discount').val();
    var checkBox = document.getElementById("myCheck");
    ...
    var linefromDate = $('input[name="linefromDate[]"]').map(function() {
        return $(this).val();
    }).get();
    var linetoDate = $('input[name="linetoDate[]"]').map(function() {
        return $(this).val();
    }).get();
    ...
    var posting = $.get(completeurl,
            {
                linecommodityName: linecommodityName,
                lineproductName: lineproductName,
                linetoDate: linetoDate,
                linefromDate: linefromDate,
                linetotal: linetotal,
...
function closeSalesUpdateRetailModal(gstType) {
    $('#mainModal').closeModal();
...
function closeUpdateHallModal(gstType) {
    $('#mainModal').closeModal();
...
function loadAvailableRoomList(roomTypeId)
{
    var fromDate = $("#fromDate").val();
    var toDate = $("#toDate").val();
    var requesturl = url + 'sales-salesmalleswara/loadAvailableRoomList';
    var data = "roomTypeId=" + roomTypeId + "&fromDate=" + fromDate + "&toDate=" + toDate;
    //$("#loadRoomList").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadRoomList');
}
function loadAvailableRoom()
{
    var roomTypeId = $("#commodityRefId").val();
    var fromDate = $("#fromDate").val();
    var toDate = $("#toDate").val();
    var requesturl = url + 'sales-salesmalleswara/loadAvailableRoomList';
    var data = "roomTypeId=" + roomTypeId + "&fromDate=" + fromDate + "&toDate=" + toDate;
...
function loadRoomId(roomNumber)
{
    alert(roomNumber);
    var requesturl = url + 'sales-salesmalleswara/loadRoomId';
    var data = "roomNumber=" + roomNumber;
    //$("#loadRoomList").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadRoomIdDetai');
}
function updateRetailInvoice() {
    ...
    if (paymentMode == 3) {        
        var advancePaymentBank = $("#advancePaymentBank").val();
        var advancePaymentCash1 = $("#advancePaymentCash").val();
        var advancePaymentCash = parseFloat(advancePaymentCash1) + parseFloat(paidAmount);
        advancePayment = parseFloat(advancePaymentBank) + parseFloat(advancePaymentCash);
    } else if (paymentMode == 2) {
        advancePaymentBank = $("#advancePayment").val();
        advancePaymentCash1 = 0;
        var advancePaymentCash = parseFloat(advancePaymentCash1) + parseFloat(paidAmount);
        advancePayment = $("#advancePayment").val();
    } else {
        advancePaymentBank = 0;
        advancePaymentCash1 = $("#advancePayment").val();
        var advancePaymentCash = parseFloat(advancePaymentCash1) + parseFloat(paidAmount);
        advancePayment = advancePaymentCash;
    }
    var lineproductName = $('input[name="lineproductName[]"]').map(function() {
        return $(this).val();
    }).get();
    ...
    var grandTotal = parseFloat(grandTotal1) + parseFloat(paidAmount);
    var billId = $("#billId").val();
    $('#mainModal').openModal({dismissible: false});
    var completeurl = url + "sales-salesmalleswara/updateRetailSalesInvoiceHall";
    var place = "mainModal";
    $("#" + place).html("Loading");
    var posting = $.get(completeurl,
            {
...
function availableRoomType() {
    var select = document.getElementById('commodityRefId');
    for (var increment = 0; increment < availableRooms.item.length; increment++) {
        var opt = document.createElement('option');
        opt.value = availableRooms.item[increment].roomSpecificationId;
        opt.innerHTML = availableRooms.item[increment].roomSpecificationTypeName;
        select.appendChild(opt);
    }
}
function availableTypeDetail()
{
    var completeurl = url + 'item-item/availableTypeDetail';
    console.log(completeurl);
    var data = "";
    availableRooms = ajaxloadwithresponses('POST', completeurl, data);
}
function loadperDay() {
    var commodityRefId = $("#commodityRefId").val();
    for (var i = 0; i < availableRooms.item.length; i++) {
        if (availableRooms.item[i].roomSpecificationId === commodityRefId) {
            if ($("#billGSTType").val() == "1") {
                $("#cgstRate").val(availableRooms.item[i].cgstRate);

                $("#sgstRate").val(availableRooms.item[i].sgstRate);
                $("#igstRate").val("0.0");
            } else
            {
                $("#cgstRate").val("0.00");
                $("#sgstRate").val("0.00");
                $("#igstRate").val(availableRooms.item[i].igstRate);
            }
            $("#unitRate").val(availableRooms.item[i].roomRentPerDay);
            $("#cgstRate").val(availableRooms.item[i].cgstRate);
            $("#sgstRate").val(availableRooms.item[i].sgstRate);
            $("#igstRate").val(availableRooms.item[i].igstRate);
            $("#hsnCode").val(availableRooms.item[i].hsnCodeRefId);
            $("#UOM").val($("#uomGold").val());
            $("#unitRate").siblings("label,i").addClass("active");
            return false;
        }
...
function billItemSave() {
    ...
    var productName = $("#salesproductName option:selected").text();
    var commodityRefId = $("#commodityRefId").val();
    var commodityName = $("#commodityRefId option:selected").text();
    ...
    var UOM = $("#UOM").val();
    var hsnCode = $("#hsnCode").val();
    //var noDays=Math.round((toDate-fromDate)/(1000*60*60*24));
    var oneDay = 24 * 60 * 60 * 1000;
    var firstDate = new Date(fromDate);
    //var startDate = firstDate.toString().replace(/[-]/, ",");
    //alert(startDate);
    var secondDate = new Date(toDate);
    var diffDays = Math.round(Math.abs((firstDate.getTime() - secondDate.getTime()) / (oneDay)));
    //var difference_ms = Math.abs(fromDate.getTime() - toDate.getTime());
    //var noDays = Math.ceil(difference_ms / (1000 * 3600 * 24));
    //alert(noDays);
    ...
    var total = parseFloat(noDays) * parseFloat(unitRate);
//alert(total);
    var cgstdisplay;
    var sgstdisplay;
    var igstdisplay;
    ...
    var billitemrow = '<div class="input-field col s12" id="billItemRow' + rowcount + '">\n\
                        <div class="input-field col s2">\n\
                         <input readonly  name="linecommodityRefId[]" type="hidden" value="' + commodityRefId + '">\n\
                         <input readonly  name="linecommodityName[]" type="text" value="' + commodityName + '">\n\
                         </div>\n\
<div class="input-field col s2">\n\
                         <input readonly name="lineproductId[]" type="hidden" value="' + productId + '">\n\
                         <input readonly  name="lineproductName[]" type="text" value="' + productName + '">\n\
                        ...
                        <input readonly  name="lineigstRate[]" type="text" value="' + igstRate + '">\n\
                         ...
                         \n\<input readonly  name="lineunitrate[]" type="text" value="' + unitRate + '">\n\</div>\n\
                         \n\<div class="input-field col s2">\n\
                         \n\<input readonly  name="linefromDate[]" type="text" value="' + fromDate + '">\n\</div>\n\
                         \n\<div class="input-field col s2">\n\
                         \n\<input readonly  name="linetoDate[]" type="text" value="' + toDate + '">\n\</div>\n\
                         \n\<div class="input-field col s1">\n\
                         \n\ \n\<input readonly  name="linequantity[]" type="text" value="' + noDays + '">\n\</div>\n\
                         \n\<div class="input-field col s1">\n\
                         \n\<input readonly  name="linetotal[]" type="text" value="' + total + '">\n\
                         \n\<input readonly  name="lineUOM[]" type="hidden" value="' + UOM + '">\n\
                         \n\ <input readonly  name="linehsnCode[]" type="hidden" value="' + hsnCode + '">\n\
                         </div>\n\
                        <div class="input-field col s1">\n\
                         <i class="mdi-action-delete red darken-1" onclick="removeItem(' + rowcount + ')"></i>\n\
                        </div>\n\
...
function closeAddRetailDetails() {
    $('#mainModal').closeModal();
...
function closeUpdateRetailSale() {
    $('#mainModal').closeModal();
    requesturl = url + 'sales-sales/updateRetailSales';
    loader();
    ajaxload('GET', requesturl, '', processor);
}
function printRetailDetails(billId)
{
    var data = "billId=" + billId;
    var completeurl = url + 'sales-salesmalleswara/printRetailDetails?' + data;
    window.open(completeurl);
}
function loadRoomDetails(salesBillID)
{
    var requesturl = url + 'sales-salesmalleswara/loadRoomDetails';
    var data = "salesBillID=" + salesBillID;
    $("#loadRoomDetails").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
    ajaxload('GET', requesturl, data, 'loadRoomDetails');
}
function CheckIn(salesBillId, billRefId, roomType, roomNumber) {
    var checkinTime = $("#appt-time").val();
    event.preventDefault();
    $('#mainModal').openModal({dismissible: false});
    var completeurl = url + "sales-salesmalleswara/CheckIn";
    var place = "mainModal";
    ;
    $("#" + place).html("Loading");
    var posting = $.get(completeurl,
...
function closeCheckInDetails(salesBillID) {
    $('#mainModal').closeModal();
    var requesturl = url + 'sales-salesmalleswara/loadRoomDetails';
    var data = "salesBillID=" + salesBillID;
    $("#loadRoomDetails").html('<div style="text-align:center;padding:0% 0%;"><img style="width:100px;" src="' + url + 'assets/img/loading.gif"/><br/><label>Please Wait...</label></div>');
...
function CheckOut(billRefId, salesBillId, roomType, roomNumber) {
    var rowCount = $('input[name="rowCount[]"]').map(function() {
        return $(this).val();
    }).get();
    var totalRowCount = rowCount.length;
    if (totalRowCount == 1) {
        var salesUpdateFlag = 1;
    } else {
        salesUpdateFlag = 0;
    }
    var checkinTime = $("#appt-time").val();
    event.preventDefault();
    $('#mainModal').openModal({dismissible: false});
    var completeurl = url + "sales-salesmalleswara/CheckOut";
    var place = "mainModal";
    ;
    $("#" + place).html("Loading");
    var posting = $.get(completeurl,
            {
                ...
                roomNumber: roomNumber,
                checkinTime: checkinTime,
                salesUpdateFlag: salesUpdateFlag,
                billRefId: billRefId
            });
