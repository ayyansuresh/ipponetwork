    ...
    if (productId == "") {
        ...
        for (var i = 0; i < loadedItemName.item.length; i++) {
            if (loadedItemName.item[i].ItemId === productId) {
                ...
                $("#unitRate").val(loadedItemName.item[i].UnitPriceWholeSale);
                ...
                //  var purchaseQuantity = parseFloat(loadedItemName.item[i].purchaseQuantity);
                //  var salesQuantity = parseFloat(loadedItemName.item[i].salesQuantity);
                //  var openingUOMQuantity = parseFloat(loadedItemName.item[i].closingUOMQuantity);
                var availableQuantity = parseFloat(loadedItemName.item[i].closingUOMQuantity);

                $("#availableQuantity").html(availableQuantity);
                // $("#unitRate").siblings("label,i").addClass("active");
                // $('#unitRate').focus();
                return false;
            }
...
function purchasebillItemSave() {
    ...
    var billitemrow = '<strong><div class="col s12" id="billItemRow' + rowcount + '">\n\
                        <div class="col s3">\n\
...
function billItemSave() {
    ...
    var unitRate = parseFloat(unitRateOrginal)
            * parseFloat(billFactor) * 100 / (100 + parseFloat(cgstRate) +
            parseFloat(sgstRate) + parseFloat(igstRate));
    ...
    var billitemrow = '<strong><div class="col s12" id="billItemRow' + rowcount + '">\n\
                        <div class="col s3">\n\
                        <input readonly name="lineproductId[]" type="hidden" value="' + productId + '">\n\
                        <input name="lineproductName[]" type="text" value="' + productName + '">\n\
                        </div>\n\
                        <div class="col s1">\n\
                        <input readonly  name="hsnCode[]" type="text" value="' + hsnCode + '">\n\
                        </div>\n\
                        <div class="col s1" ' + cgstdisplay + '>\n\
                        <input readonly  name="linecgstRate[]" type="text" value="' + cgstRate + '">\n\
                       </div><div class="col s1" ' + sgstdisplay + '>\n\
                        <input readonly  name="linesgstRate[]" type="text" value="' + sgstRate + '">\n\
                        </div>\n\
                        <div class="col s1" ' + igstdisplay + '>\n\
                        <input readonly  name="lineigstRate[]" type="text" value="' + igstRate + '">\n\
                        ...
                        <div class="col s1">\n\
                         <input readonly  name="lineunitrate[]" type="hidden" value="' + unitRate.toFixed(2) + '">\n\
      <input readonly  name="lineunitratewithtax[]" type="text" value="' + unitRateWithTax.toFixed(2) + '">\n\
                        </div>\n\
                        <div class="col s1">\n\
                         <input readonly  name="linequantity[]" type="text" value="' + quantity + '">\n\
                         <input readonly  name="lineUOM[]" type="hidden" value="' + UOM + '">\n\
        ...
        for (var i = 0; i < loadedItemName.item.length; i++) {
            ...
            if (loadedItemName.item[i].ItemId === productId || loadedItemName.item[i].barCode === productId) {
                ...
                var unitRateOriginal = (parseFloat(loadedItemName.item[i].UnitPrice))
                        * 100 /
                        (100 + parseFloat(loadedItemName.item[i].cgstRate) +
                                parseFloat(loadedItemName.item[i].sgstRate)
                                );
...
function updateRetailSalesInvoice() {
    ...
    //   billGSTType = $("#billGSTType").val();
    //  closeRetailSalesModal(billGSTType);
}
...
function addField(argument) {
    ...
    if (barcodeId !== "") {
        ...
        if (itemFlag == 0)
        {
            ...
            barcodeId.setAttribute("id", "barcodeId" + currentIndex);
            barcodeId.setAttribute("onchange", functionName);
            barcodeId.setAttribute("tabindex", currentIndex + 1);
            barcodeId.setAttribute("autofocus", "");
            ...
            quantity.setAttribute("value", "1");
            quantity.setAttribute("id", "Qty" + currentIndex);
            quantity.setAttribute("tabindex", currentIndex + 2);
            //$('#productId' + currentIndex).focus();
            var functionName = "finalTotal(" + currentIndex + ");addField(" + currentIndex + ");";
...
function makeRetailSalesInvoicePrint(printpage, company, accountYear) {
    var lineproductid = $('input[name="lineproductId[]"]').map(function () {
        return $(this).val();
    }).get();
    //event.preventDefault();
    if ($("#customerName").val() == "" && $("#billType").val() == 2) {
...
function updateRetailSalesInvoicePrint(printpage, company, accountYear) {
    event.preventDefault();
    var linenumberofbags = $('input[name="linenumberofbags[]"]').map(function () {
        return $(this).val();
    }).get();
...
//    printNewUpdate(printpage, company, accountYear, billNumber);
}
function printdiv(printpage, gstType, company, accountYear)
{
    ...
    var data = url + 'sales-salesmalleswara/generateInvoicePdf?  &company=' + company + '&accountYear=' + accountYear +
...
function printNew(printpage, company, accountYear, billNumber)
{
...
function printNewUpdate(printpage, company, accountYear, billNumber)
{
    // alert(billNumber1);
    var gstType = 3;
    var billType = 3;
...
function printNewReport(printpage, company, accountYear)
{
    var billNumber = $('#frombillnumber').val();
    var gstType = 3;
    var billType = $("#billType").val();
    var completeurl = url + "sales-salesmalleswara/retailPdf";
    var data = ' &company=' + company + '&accountYear=' + accountYear +
