...
//alert(unitRateOrginal);
    ...
    var billitemrow = '<div class="input-field col s12" id="billItemRow' + rowcount + '">\n\
                      ...
                      <input readonly  name="linepackingfactor[]" type="hidden" value="' + packingFactor + '">\n\
                        ...
                        <div class="input-field col s1">\n\
                          <input readonly  name="linediscount[]" type="text" value="' + discount + '">\n\
                        </div>\n\
                        <div class="input-field col s1">\n\
...
function makePurchaseInvoice() {
    ...
    if (parseFloat($("#grandTotal").val()) > 10000 && ((($("#billType").val()) == 2) || ($("#billType").val()) == 3)) {
        ...
        // Calculate unit rate with tax for each line item
        var unitRateWithTax = linerate.map(function (rate, index) {
            var baseRate = parseFloat(rate) || 0; // Ensure rate is a number, default to 0 if invalid
            var cgstRate = parseFloat(linecgstrate[index]) || 0; // CGST rate (percentage)
            var sgstRate = parseFloat(linesgstrate[index]) || 0; // SGST rate (percentage)
            var igstRate = parseFloat(lineigstrate[index]) || 0; // IGST rate (percentage)
            var taxAmount = 0;
            if (igstRate > 0) {
                taxAmount = (baseRate * igstRate) / 100;
            } else {
                taxAmount = (baseRate * (cgstRate + sgstRate)) / 100;
            }
            return (baseRate + taxAmount).toFixed(2); 
        });
        
        // Calculate line product total for each line item (unitRateWithTax * quantity)
        var lineproducttotal = unitRateWithTax.map(function (rateWithTax, index) {
            var quantity = parseFloat(linequantity[index]) || 0; 
            return (parseFloat(rateWithTax) * quantity).toFixed(2); 
        });

         // Calculate overall total (sum of line product totals)
        var overalltotal = lineproducttotal.reduce(function (sum, total) {
        ...
        var posting = $.get(completeurl,
                {
                    ...
                    st_lineuomId: lineUOM,
                    st_linepackingFactor : 1,
                    st_linecommodityId: linecommodityRefId,
                    st_lineproductid: lineproductid,
                    st_lineqty: linequantity,
                    st_lineproductprice: unitRateWithTax,
                    st_linetotal: lineproducttotal,
                    st_overallTotal : overalltotal,
                    st_date : $("#billDate").val(),
                    st_fromcustomersiteid : 0,
                    st_tocustomersiteid :  $("#customerSiteName").val(),
                    st_description : $("#journalEntryDescription").val()
        ...
        var posting = $.get(completeurl,
                {
                    ...
                    villagecustomerName: $("#villagecustomerName").val(),
                    villagecustomerCity: $("#villagecustomerCity").val(),
                    villagecustomerId: $("#villagecustomerId").val(),
                    linenumberofbags: linenumberofbags,
                    billId: billId,
                    recieveDate:$("#recieveDate").val(),
                    reverseCharge: $("#reverseCharge").val(),
        ...
        for (var i = 0; i < loadedItemName.item.length; i++) {
            if (loadedItemName.item[i].ItemId === productId || loadedItemName.item[i].barCode === productId) {
             ...
             // $("#Qty" + rowindex).val(loadedItemName.item[i].Quantity);
              $("#itemId" + rowindex).val(loadedItemName.item[i].ItemId);
...
function addField(argument) {
    ...
    itemId.setAttribute("name", "lineitemId[]");
    itemId.setAttribute("type", "hidden");
    itemId.setAttribute("id", "itemId" + currentIndex);
    itemId.setAttribute("readonly", "");
