...
$creditResult = stockBlock::getNewCreditResult($companyID, $accountYear);
$outPassCreditResult = stockBlock::getVendorCreditResult($companyID, $accountYear);
$debitResult = stockBlock::getNewDebitResult($companyID, $accountYear);
$outPassDebitResult = stockBlock::getVendorDebitResult($companyID, $accountYear);
$arrayCount = 0;
...
foreach ($outPassCreditResult as $outPassCredit) {
    $outPassCredit = (array) $outPassCredit;
    $commodityId = $outPassCredit[commodity_id];
    $outPassCreditList[$commodityId]['Credit'] = $outPassCredit['Credit'];
}
...
foreach ($outPassDebitResult as $outPassDebit) {
    $outPassDebit = (array) $outPassDebit;
    $commodityId = $outPassDebit[commodity_id];
    $outPassDebitList[$commodityId]['Debit'] = $outPassDebit['Debit'];
}
?>
                    ...
                    <table id="data-table-reports" class="responsive-table display">
                        ...
                        <tbody>
                            ...
                            for ($increment = 0; $increment < count($commodityList); $increment++) {
                                ...
                                $creditNew = $outPassCreditList[$commodityId]['Credit'];
                                $debit = $debitList[$commodityId]['Debit'];
                                $debitNew = $outPassDebitList[$commodityId]['Debit'];
                                $trial = $openingStock + $credit + $creditNew - $debit - $debitNew;
                                ?>
                                <tr>
                                    ...
                                    <td> <?php echo $creditNew; ?></td>
                                    <td> <?php echo $debit; ?></td>
                                    <td> <?php echo $debitNew; ?></td>
