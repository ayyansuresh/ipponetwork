...
$commodityResult = stockBlock::getItemStock($companyID, $accountYear);
$creditResult = stockBlock::getItemCreditResult($companyID, $accountYear);
$debitResult = stockBlock::getItemDebitResult($companyID, $accountYear);
$arrayCount = 0;
foreach ($commodityResult as $commodityOpening) {
    $commodityOpening = (array) $commodityOpening;
    $commodityId = $commodityOpening[items_item_id];
    ...
    $openingList[$commodityId]['commodityName'] = $commodityOpening[items_name];
...
foreach ($creditResult as $credit) {
    $credit = (array) $credit;
    $commodityId = $credit[items_item_id];
    $creditList[$commodityId]['Credit'] = $credit['Credit'];
}
foreach ($debitResult as $debit) {
    $debit = (array) $debit;
    $commodityId = $debit[items_item_id];
    $debitList[$commodityId]['Debit'] = $debit['Debit'];
}
?>
<div class="container teal lighten-2">
    <div class="collection">
        <h4 class="collection-item teal darken-2 center-align" style="color:#fff !important;">Item Stock Reports</h4>
    </div>
            ...
            <div id="admin" class="col s12">
                <div class="card material-table">
                    <div class="input-field col s12 m4 right" style="text-align: right;">
                        <button class="waves-effect waves-light btn teal darken-2" onclick="printItemStockReport();"><i class="mdi-av-my-library-books left"></i> Export to PDF</button>
                    </div>
