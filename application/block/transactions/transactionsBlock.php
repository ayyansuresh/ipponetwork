...
class transactionsBlock extends Controller {
    ...
    public static function getSalesBillByCustomer() {
        $option = "";
        $SalesBillDetail = transactionsModel::getSalesBillByCustomer();
        foreach ($SalesBillDetail as $SalesBill) {
            $SalesBill = (array) $SalesBill;
            $option = $option . '<option value="' . $SalesBill[salesbill_sales_bill_id] . '">' . $SalesBill[salesbill_sales_bill_display_number] . '</option>';
        }
        ?>
        <div class="input-group" >
            <label for="SalesBillId" >Choose Sale Bill</label>
            <div class="sel-wrap">
                <select id="SalesBillId" class="floating-label" data-validation="select"
                        data-content="Please Select Sales Bill">
                    <option value=""  <?php echo $enabled; ?>>Select Sales Bill</option>
                    <?php echo $option; ?>
                </select>
                <div class='bar'></div>
            </div>  
    ...
    public static function getPurchaseBillByCustomer() {
        $option = "";
        $PurchaseBillDetail = transactionsModel::getPurchaseBillByCustomer();
        foreach ($PurchaseBillDetail as $PurchaseBill) {
            $PurchaseBill = (array) $PurchaseBill;
            $option = $option . '<option value="' . $PurchaseBill[purchasebill_purchase_bill_id] . '">' . $PurchaseBill[purchasebill_purchase_bill_display_number] . '</option>';
        }
        ?>
        <div class="input-group" >
            <label for="PurchaseBillId" >Choose Purchase Bill</label>
            <div class="sel-wrap">
                <select id="PurchaseBillId" class="floating-label" data-validation="select"
                        data-content="Please Select purchase Bill">
                    <option value=""  <?php echo $enabled; ?>>Select Purchase Bill</option>
                    <?php echo $option; ?>
                </select>
                <div class='bar'></div>
            </div>  
        ...
        <script>
            floatingSelect2('PurchaseBillId');
        </script>
        <script type="text/javascript" src="<?php echo URL; ?>assets/js/materialize/plugins.js"></script>
        <link rel="stylesheet" href="<?php echo URL; ?>assets/css/select2/custom.css">
        <?php
    ...
    public static function getCustomerNameWithCityOnlySalesorPurchaseSales() {
        $option = "";
        $customerNameDetail = transactionsModel::getCustomerNameWithCityOnlySalesorPurchaseSalesByCompanyId();
        foreach ($customerNameDetail as $customerName) {
            $customerName = (array) $customerName;
            $option = $option . '<option value="' . $customerName[customer_id] . '">' . $customerName[customer_name] . '-' . $customerName[city_name]. '</option>';
        }
    ...
    public static function getCustomerNameWithCityOnlyPurchaseorPurchaseSalesByCompanyId() {
        $option = "";
        $customerNameDetail = transactionsModel::getCustomerNameWithCityOnlyPurchaseorPurchaseSalesByCompanyId();
        foreach ($customerNameDetail as $customerName) {
            $customerName = (array) $customerName;
            $option = $option . '<option value="' . $customerName[customer_id] . '">' . $customerName[customer_name] . '-' . $customerName[city_name]. '</option>';
        }
    ...
    public static function makeCustomerCreditNoteTransaction() {
        return transactionsModel::makeCustomerCreditNoteTransaction();
    }
    ...
    public static function makeCustomerDebitNoteTransaction() {
        return transactionsModel::makeCustomerDebitNoteTransaction();
    }
    ...
    public static function getCreditNoteDetails() {
        return transactionsModel::getCreditNoteDetails();
    }
    
    public static function getDebitNoteDetails() {
        return transactionsModel::getDebitNoteDetails();
    }
