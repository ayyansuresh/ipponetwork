...
class salesInvoiceBlock extends Controller {
    ...
    public static function getRetailDetails() {
        return salesModel::getRetailDetails();
    }

    public static function printRetailDetails() {
        $company = $_SESSION['beebooklogincompanyid'];
        $accountyear = $_SESSION['beebookloginaccountyearid'];

        $billNumber = generalhelper::getGetElement('billId');
        $data = "";
        $url = URL1 . 'sales-salesmalleswara/loadRetailDetailsPdfPrint?billId=' . $billNumber;
        $html = file_get_contents($url);
        $head = "";
        $footer = "";
        generalhelper::pdfRetailDetails($html, $head, $footer, 'Quotation', $billNumber);
    }

    public static function getRetailGridDetails() {
        return salesModel::getRetailGridDetails();
    }

    public static function getRoomDetails() {
        return salesModel::getRoomDetails();
    }

    public static function RoomDetails() {
        return salesModel::RoomDetails();
    }

    public static function CheckIn() {
        return salesModel::CheckIn();
    }

    public static function CheckOut() {
        return salesModel::CheckOut();
    }
