...
class salesInvoiceBlock extends Controller {
    ...
    /* public static function generatePdfQuote() {
      ...
      $url = URL . 'sales-salesmalleswara/printPdf?company=' . generalhelper::getGetElement("company") .
...
public static function generatePrintPdfSample() {
        //$billNumber = generalhelper::getGetElement('invoiceNumber');
        $company = $_SESSION['beebooklogincompanyid'];
        $accountyear = $_SESSION['beebookloginaccountyearid'];
        ...
        $data = array("company" => $company,
            ...
            "tobillnumber" => $tobillnumber,
            "billType" => $billType);
        //$url = URL1 . 'sales-sales/printDeliveryPdfSample?company=' . $company . '&accountYear=' . $accountyear . '&gstType=' . $gstType .
                '&frombillnumber=' . $frombillnumber . '&tobillnumber=' . $tobillnumber . '&billType=' . $billType;
    ...
    public static function getBillValue() {
        $billPrefixResult = salesModel::getBillValue();
        $billPrefix = (array) $billPrefixResult[0];
        return $billPrefix;
    }
      public static function getDeliveryBillDetails() {
        $lastBillNumber = salesModel::getLastDeliveryBillDetail();
        if ($lastBillNumber == "") {
            return 1;
        } else {
            return $lastBillNumber + 1;
