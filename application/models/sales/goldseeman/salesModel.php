...
//$commit = self::updateSalesBillTagItems();
    ...
    public static function updateSalesBillTagItemsStatus() {
        ...
        try {
            ...
            $tagItemsRefId = implode("','", $tagItemsId);
...
// $billDisplay = $billNumberDisplay[sales_prefix_value] . $displayNumber;
            ...
            if (generalhelper::getGetElement('billType') == 3 || generalhelper::getGetElement('billType') == 2) {
                $sql = "insert into " . table_village_customer . "(" . village_billType . ","
                        . village_customerName . "," . village_customerTown
                        . "," . village_customeraddress . "," . village_pannumber
                        . "," . village_aadharnumber . "," . village_billRefId . "," . village_mobilenumber . "," . village_tableDetailId
                        . ")"
                        . " values (:" . village_billType
                        . ",:" . village_customerName . ",:" . village_customerTown
                        . ",:" . village_customeraddress . ",:" . village_pannumber . ",:" . village_aadharnumber .
                ...
                $parameter = array(':' . village_billType => salesBill,
                    ...
                    ':' . village_billRefId => self::$salesBillId,
                    ':' . village_mobilenumber => generalhelper::getGetElement('mobileNumber'),
                    ':' . village_tableDetailId => salesBill
                );
                $query->execute($parameter);
            }
        } catch (PDOException $ex) {
    ...
    public static function saveInvoiceItems($taxFlag) {
        ...
        try {
            ...
            $linebarcodeId = generalhelper::getGetElementArray('linebarcodeId');
            ...
            $datafields = array(salesbillitemgold_sales_bill_ref_id, salesbillitemgold_sales_bill_date,
                ...
                salesbillitemgold_vad, salesbillitemgold_makingCharge, salesbillitem_tagRefNumber,
                salesbillitemgold_total_withtax, salesbillitemgold_unitrate_wittax, salesbillitemgold_lessWeight, salesbillitem_netWeight
            );
            for ($increment = 0; $increment < count($linetotal); $increment++) {
                ...
                if ($lineproductId[$increment] != 0) {
                    $datafieldsValue = array(salesbillitemgold_sales_bill_ref_id => self::$salesBillId,
                        ...
                        salesbillitem_tagRefNumber => $linebarcodeId[$increment],
                        ...
                        salesbillitemgold_lessWeight => $lineLessWeight[$increment],
                        salesbillitem_netWeight => $lineNetWeight[$increment]
                    );
                    $insert_values = array_merge($insert_values, array_values($datafieldsValue));
                    $question_marks[] = '(' . generalhelper::placeholders('?', sizeof($datafields)) . ')';
                }
    ...
    public static function getBillDetailsByNumber() {
        ...
        $sql = "select a.*,b.* from " . table_salesbill_gold . " as a " .
                ...
                village_billRefId . " and b." .village_billRefId. " = " .salesBill.
    ...
    public static function getBillItem($billId) {
        $sql = "select a.*,b.*,c.*,d." . salesbilltagitemes_id . " from " . table_salesbillitem_gold . " as a " .
                " inner join " . table_items . " as b on a." . salesbillitem_item_ref_id . " = b." . items_item_id .
                " left join " . table_salesbilltagitemes . " as d on a." . salesbillitem_tagRefNumber . " = d." . salesbilltagitemes_tagNumber .
    ...
    public static function getReceiptDetailsByIdGold() {
        $billId = generalhelper::getGetElement('salesBillId');
        echo $sql = "select a." . salespayment_receipt_number . ",a." . salespayment_id . ", a." .
...
//$commit = self::updateSalesBillTagItems();
        ...
        // }
        if ($commit == 1) {
            $commit = self::updateSalesBillTagItemsStatus();
        }
        if ($commit === 1) {
            if (generalhelper::getGetElement('billUpdateFlag') == 0) {
...
//. " and a." . salesbill_vat_cst_flag . " = " . $vatCstFlag;
    ...
    public static function getSalesDashboardReportTotal($year) {
        $sql = ' select sum(a.runningTotal) as monthsales,'
...
WHERE MONTH(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =  ' . $previousMonth
    ...
    public static function getMonthlySalesTotalAll($curentMonth, $curentYear) {
        ...
        $sql = ' select sum(runningTotal) as monthsales,MONTH(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) as month,MONTHNAME(salesBillDate) as monthname ,
"current" as salesmonth from salesbill 
WHERE MONTH(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =  ' . $curentMonth .
                ' and YEAR(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =  ' . $curentYear . '    
UNION 
...
WHERE MONTH(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =  ' . $previousMonth
 ...
 and YEAR(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) = ' . $curentYear
    ...
    public static function getTotalProfitCurrentMonth($month, $year) {
        $sql = 'select sum( (round(total) - (round((Quantity +round((Quantity * 8/100),3))*unitrate,2)) )) as profit
 FROM salesbillitem where  MONTH(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =' . $month . ' and YEAR(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =' . $year . '
and vad >8  GROUP BY ID
UNION
select sum( (  (round(makingCharge+(Quantity +round((Quantity * vad/100),3))*unitrate,2)) - (round((Quantity +round((Quantity * 8/100),3))*unitrate,2)))) as profit
FROM salesbillitem where  MONTH(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =' . $month . ' and YEAR(STR_TO_DATE(salesBillDate,"%Y-%m-%d")) =' . $year . '
and vad <=8 and((round((Quantity +round((Quantity * 8/100),3))*unitrate,2))  < (round(makingCharge+(Quantity +round((Quantity * vad/100),3))*unitrate,2)))
GROUP BY ID';
...
public static function saveOrderInvoice() {
        self::$db->beginTransaction();
        $commit = self::addSalesOrderBill();
        $billType = generalhelper::getGetElement('billType');
        $taxFlag = generalhelper::getGetElement('taxFlag');
        if ($commit === 1) {
            $commit = self::saveInvoiceOrderItems($taxFlag);
        }
        /*if ($commit == 1) {
            $salesBillItemLastId = self::$db->lastInsertId();
            self::$salesBillItemLastId = $salesBillItemLastId;
        ...
        if ($commit === 1) {
            $commit = self::saveInvoiceOrderItemsPurchase($taxFlag);
        }
        /*if ($commit == 1) {
            $salesBillItemPurchaseLastId = self::$db->lastInsertId();
            self::$salesBillItemPurchaseLastId = $salesBillItemPurchaseLastId;
    ...
    public static function addSalesOrderBill() {
        $commit = 1;
        try {
            $billNumber = self::getEstimateLastBillNumber(generalhelper::getGetElement('billGSTType')) + 1;
            ?>
    ...
    public static function saveInvoiceOrderItems($taxFlag) {
        $commit = 1;
        try {
            $linetotal = generalhelper::getGetElementArray('linetotal');
            if($taxFlag == "1"){
    ...
    public static function saveInvoiceOrderItemsPurchase($taxFlag) {
        $commit = 1;
        try {
            $linetotal = generalhelper::getGetElementArray('lineamountpurchase');
            if($taxFlag == "1"){
