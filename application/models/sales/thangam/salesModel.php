...
where c." . salesbill_sales_bill_id . "=" . $salesBillId;
    ...
    public static function updateInvoice() {
        ...
        if ($commit === 1) {
            if (generalhelper::getGetElement('billUpdateFlag') == 0) {
                $commit = self::addSasaveRetailInvoicelesBill();
            } else {
    ...
    public static function addSalesBill() {
        ...
        try {
            ...
            <?php
            $billNumberDisplay1 = self::getBillPrefix(generalhelper::getGetElement('billGSTType'));
            $billNumberDisplay = (array) $billNumberDisplay1[0];
            ...
            $sql = "insert into " . table_sales_bill . "(" . salesbill_sales_bill_number . ","
                    ...
                    . "," . salesbill_address_id . "," . salesbill_account_ref_id . "," . salesbill_vat_cst_flag . "," . salesbill_creditPoint
                    . ")"
            ...
            $parameter = array(
                ...
                ':' . salesbill_creditPoint => generalhelper::getGetElement('grandTotal') / 50
            );
    ...
    public static function getSalesInvoiceDetailsRetail() {
        ...
        $company = generalhelper::getGetElement('company');
        $accountyear = generalhelper::getGetElement('accountYear');
        $gsttype = generalhelper::getGetElement('billType');
        $sql = "SELECT a." . salesbill_sales_bill_id . ",a." . salesbill_sales_bill_display_number . ",a." . salesbill_creditPoint
                . ",a." . salesbill_sales_bill_number . ",a." . salesbill_sales_bill_date . ",a." . salesbill_transport .
                ",a." . salesbill_bundle . ",a." . salesbill_product_discount . ",a." . salesbill_total_discount .
                ",a." . salesbill_cgst_total . ",a." . salesbill_sgst_total . ",a." . salesbill_igst_total . ",a." . salesbill_chess_rate . ",a." . salesbill_total_chess .
                ",a." . salesbill_running_total . ",a." . salesbill_fright . ",a." . salesbill_round_off . ",a." . salesbill_sales_bill_total . ",a." . salesbill_sales_bill_type .
                ",a." . salesbill_sales_bill_status . ",a." . salesbill_sales_bill_stage . ",a." . salesbill_gst_type .
                ",b." . village_party_id . ",b." . village_customerName . ",b." . village_customerTown . ",f." . company_name_tamil .
                ",f." . company_name_english . ",f." . company_name_tamil .
                ",h.* FROM " . table_sales_bill . " as a
                left JOIN " . table_village_customer . " as b ON b." . village_billRefId . " = a." . salesbill_sales_bill_id
                . " inner JOIN " . table_company . " as f ON f." . company_id . " = a." . salesbill_company_ref_id . " 
                ...
                " and (a." . salesbill_sales_bill_type . "=" . 2 . " or a." . salesbill_sales_bill_type . "=" . 3 .
                ") and (" . salesbill_sales_bill_number . ">=" . $frombillnumber . " and " . salesbill_sales_bill_number .
                " <=" . $tobillnumber . ") order by a." . salesbill_sales_bill_display_number;
        $query = self::$db->prepare($sql);
        $query->execute();
    ...
    public static function getSalesInvoiceRetailDetails() {
        ...
        $sql = "SELECT a.*,b.*,c.* FROM " . table_sales_bill . " as a
                left JOIN " . table_customer . " as b ON b."
                . customer_id . " = a." . salesbill_customer_id .
                " left JOIN " . table_customer_address . " as c ON b." . customer_id . " = c."
                . customeraddress_customer_ref_id .
                "   WHERE a." . salesbill_company_ref_id . "  = " . $company . " and a." . salesbill_account_year_ref_id . "=" . $accountyear .
                " and " . salesbill_sales_bill_number .
                " =" . $tobillnumber . " order by a." . salesbill_sales_bill_display_number;
        $query = self::$db->prepare($sql);
        $query->execute();
    ...
    public static function saveRetailInvoice() {
        ...
        if ($commit == 1) {
            if ($billType != 3) {
                $Number = self::getMobileNumber();
                $message = "தங்களின் பில் தொகை ரூபாய் " . generalhelper::getGetElement('grandTotal') . "/-  தங்கம் 916 செக்கு ஆயில்";
                generalhelper::sendsms($message, $Number);
            }
            if ($billType == 3 & generalhelper::getGetElement('villagecustomerCity') != "") {
                $message = "தங்களின் பில் தொகை ரூபாய் " . generalhelper::getGetElement('grandTotal') . "/-  தங்கம் 916 செக்கு ஆயில்";
                generalhelper::sendsms($message, generalhelper::getGetElement('villagecustomerCity'));
            }
    ...
    public static function getCustomerCreditPoint($customerid, $billdate) {
        ...
        $sql = "select a.*,b.*,c.*,d.*,e.*, SUM(f.creditPoint) as totalpoint,COUNT(f.salesBillID) as salesbillcount,SUM(f.salesBillTotal) as salestotal from " . table_customer . " as a LEFT JOIN " . table_customer_address . " as b on a. " . customer_id . " = b. " . customeraddress_address_id . " LEFT join " . table_customer_type . " as c"
                ...
                " where a." . customer_id . " = " . $customerid . " and f." . salesbill_sales_bill_date . "<='" . $billdate . "'";
        $query = self::$db->prepare($sql);
        $query->execute();
        return $query->fetchAll();
    }
    ...
    public static function getMobileNumber() {
        $customer = generalhelper::getGetElement('customerName');
        $sql = "select " . customer_field3 . " as mobile from " . table_customer . " where " . customer_id . " = " . $customer;
        $query = self::$db->prepare($sql);
        $query->execute();
        return $query->fetch()->mobile;
    }
    ...
    public static function makeGiftDetail() {
        ...
        $commit = self::addgift();
        if ($commit == 1) {
            $giftid = self::$db->lastInsertId();
            $commit = self::addgiftitem($giftid);
        }
        if ($commit == 1) {
            $giftitemid = self::$db->lastInsertId();
            $commit = self::addstock($giftitemid);
        }
        if ($commit == 1) {
            self::$db->commit();
        } else {
    ...
    public static function addgift() {
        try {
            $giftdate = date('Y-m-d');
            $giftcustomer = generalhelper::getGetElement('customerId');
            $giftquantity = generalhelper::getGetElement('giftquantity');
            $totalCreditpoint = $giftquantity * 50;
            $insert_values = array();
            $datafields = array(gift_date, gift_customer_id,
                gift_credit_point, gift_account_year_id, gift_company_id
            );
            $datafieldsValue = array($giftdate, $giftcustomer, $totalCreditpoint,
                generalhelper::getSessionElement('beebooklogincompanyid'),
                generalhelper::getSessionElement('beebookloginaccountyearid')
            );
            $insert_values = array_merge($insert_values, array_values($datafieldsValue));
            $question_marks[] = '(' . generalhelper::placeholders('?', sizeof($datafields)) . ')';
            $sql = "INSERT INTO " . table_gift . "(" . implode(",", $datafields) . ") VALUES " . implode(',', $question_marks);
            $query = self::$db->prepare($sql);
            $query->execute($insert_values);
            $commit = 1;
        } catch (PDOException $ex) {
    ...
    public static function addgiftitem($giftid) {
        try {
            $giftdate = date('Y-m-d');
            $giftitemid = generalhelper::getGetElement('giftitemid');
            $giftcommodityid = generalhelper::getGetElement('giftcommodityid');
            $giftquantity = generalhelper::getGetElement('giftquantity');
            $unitprice = generalhelper::getGetElement('unitprice');
            $insert_values = array();
            $datafields = array(gift_item_date, gift_item_giftid,
                gift_item_commodity_id, gift_item_item_id,
                gift_item_account_year_id, gift_item_company_id, gift_item_quantity,
                gift_item_unitprice, gift_item_totalprice
            );
            $datafieldsValue = array($giftdate, $giftid,
                $giftcommodityid, $giftitemid,
                generalhelper::getSessionElement('beebooklogincompanyid'),
                generalhelper::getSessionElement('beebookloginaccountyearid')
                , $giftquantity, $unitprice, $unitprice * $giftquantity
            );
            $insert_values = array_merge($insert_values, array_values($datafieldsValue));
            $question_marks[] = '(' . generalhelper::placeholders('?', sizeof($datafields)) . ')';
            $sql = "INSERT INTO " . table_gift_items . "(" . implode(",", $datafields) . ") VALUES " . implode(',', $question_marks);
            $query = self::$db->prepare($sql);
            $query->execute($insert_values);
            $commit = 1;
        } catch (PDOException $ex) {
    ...
    public static function addstock($giftitemid) {
        try {
            $giftdate = date('Y-m-d');
            $giftcommodityid = generalhelper::getGetElement('giftcommodityid');
            $giftquantity = generalhelper::getGetElement('giftquantity');
            $insert_values = array();
            $datafields = array(stock_date, stock_commodity_ref_id,
                stock_UOM_id, stock_UOM_quantity, stock_company_ref_id,
                stock_account_year_ref_id, stock_type, stock_table_reference_id,
                stock_table_reference_detail_id, stock_created_by
            );
            $datafieldsValue = array($giftdate, $giftcommodityid,
                2, $giftquantity, generalhelper::getSessionElement('beebooklogincompanyid'),
                generalhelper::getSessionElement('beebookloginaccountyearid'), 2, giftitem,
                $giftitemid, generalhelper::getSessionElement('beebookloginuserid')
            );
            $insert_values = array_merge($insert_values, array_values($datafieldsValue));
            $question_marks[] = '(' . generalhelper::placeholders('?', sizeof($datafields)) . ')';
            $sql = "INSERT INTO " . table_stock . "(" . implode(",", $datafields) . ") VALUES " . implode(',', $question_marks);
    ...
    public static function getCustomerCreditPointdeduced($customerId) {
        $sql = "select sum(a.creditPointReduced) as totaldeduced from gifts as a INNER JOIN customer as b on a.customerId=b.customerID
