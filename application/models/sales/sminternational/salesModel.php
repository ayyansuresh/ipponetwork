...
where c." . salesbill_sales_bill_id . "=" . $salesBillId;
    ...
    public static function getSalesInvoiceDetails() {
        ...
        $sql = "SELECT a." . salesbill_total_currency . ",a." . salesbill_sales_bill_id . ",a." . salesbill_sales_bill_display_number
                ...
                ",b." . customer_aadharNumber . ",b." . customer_name . ",b." . customer_gst_number . ",b." . customer_party_gst_type . ",b." . customer_type . ",c.*".
                ", d." . city_name . ",e." . state_Code . ",e." . state_name . ",f." . company_name_english . ",f." . company_name_tamil .
                ",h.*,g.* FROM " . table_sales_bill . " as a inner JOIN " . table_customer . " as b ON b." . customer_id . " = a." . salesbill_customer_id . 
               " inner JOIN " . table_customer_address . " as c ON c." . customeraddress_address_id . " = a." . salesbill_address_id .
                ...
                inner join country as g on g.countryId = c.countryRefId     
    ...
    public static function getSalesInvoiceItemDetails($billId) {
        $sql = "SELECT a.*,b.*,c.*,d.*,e.* from " . table_sales_bill_item . " as a"
                ...
                . " inner join " . table_rate_of_commssion . " as e on e." . rate_id .
                " = a ." . salesbillitem_rateofcommission . ""
                . " where a." . salesbillitem_sales_bill_ref_id . " = " . $billId . " order by e." . rate_id . " , a." . salesbillitem_id;
        $query = self::$db->prepare($sql);
        $query->execute();
        return $query->fetchAll();
    }
    ...
    public static function getBillItem($billId) {
        $sql = "select a.*,b.*,c.*,d.* from " . table_sales_bill_item . " as a " .
                ...
                " inner join " . table_rate_of_commssion . " as d on a." . salesbillitem_rateofcommission . " = d." .
                rate_id .
    ...
    public static function getRetailBillDetailsByNumber() {
      ...
      echo $sql = "select a.*,b.* from " . table_sales_bill . " as a " .
                " left join " . table_village_customer . " as b on a." . salesbill_sales_bill_id . " = b." .
                village_billRefId .
                " where a." . salesbill_company_ref_id . " = " . generalhelper::getSessionElement('beebooklogincompanyid') .
                " and a." . salesbill_account_year_ref_id . " = " . generalhelper::getSessionElement('beebookloginaccountyearid') .
    ...
    //sm sales invoice process
    public static function smSaveInvoice() {
        self::$db->beginTransaction();
        $commit = self::addSmSalesBill();
        $billType = generalhelper::getGetElement('billType');
        if ($commit === 1) {
            $commit = self::saveSmInvoiceItems();
        }
        if ($commit == 1) {
            $salesBillItemLastId = self::$db->lastInsertId();
            self::$salesBillItemLastId = $salesBillItemLastId;
            $commit = self::saveSmStock();
        }
        if ($commit == 1) {
            $commit = self::saveSmDayTransaction();
        }
        if ($commit == 1 && ($billType == 2 || $billType == 3)) {
            $commit = self::saveSmAccountTransaction();
        }
        if ($commit == 1) {
            $commit = self::saveSmCustomerTransaction();
        }
        if ($commit == 1) {
            self::$db->commit();
        } else {
    ...
    public static function addSmSalesBill() {
        $commit = 1;
        try {
            if (generalhelper::getGetElement('billType') == 3) {
                $addressID = 0;
            ...
            $sql = "insert into " . table_sales_bill . "(" . salesbill_sales_bill_number . ","
                    . salesbill_sales_bill_display_number . "," . salesbill_sales_bill_date
                    . "," . salesbill_customer_id . "," . salesbill_vat_total
                    . "," . salesbill_cst_total
                    . "," . salesbill_running_total . "," . salesbill_round_off
                    . "," . salesbill_sales_bill_total . "," . salesbill_company_ref_id . "," . salesbill_account_year_ref_id
                    . "," . salesbill_created_by . "," . salesbill_created_timetamp . "," . salesbill_sales_bill_type
                    ...
                    . "," . salesbill_transport . "," . salesbill_bundle
                    . "," . salesbill_total_discount . "," . salesbill_product_discount
                    . "," . salesbill_address_id . "," . salesbill_vat_cst_flag . "," . salesbill_account_ref_id
                    . "," . salesbill_total_currency . "," . salesbill_currency_value
                    . "," . salesbill_currency_ref_id .
                    "," . salesbill_cgst_total . "," . salesbill_sgst_total . "," . salesbill_igst_total
                    . ")"
                    . " values (:" . salesbill_sales_bill_number
                    . ",:" . salesbill_sales_bill_display_number . ",:" . salesbill_sales_bill_date
                    . ",:" . salesbill_customer_id . ",:" . salesbill_vat_total
                    ...
                    . ",:" . salesbill_total_discount . ",:" . salesbill_product_discount
                    . ",:" . salesbill_address_id . ",:" . salesbill_vat_cst_flag . ",:" . salesbill_account_ref_id
                    . ",:" . salesbill_total_currency . ",:" . salesbill_currency_value . ",:" . salesbill_currency_ref_id
                    . ",:" . salesbill_cgst_total . ",:" . salesbill_sgst_total . ",:" . salesbill_igst_total . ")";
            $query = self::$db->prepare($sql);
            $parameter = array(':' . salesbill_sales_bill_number => generalhelper::getGetElement('billNumber'),
                ':' . salesbill_sales_bill_display_number => generalhelper::getGetElement('billNumberDisplay'),
                ':' . salesbill_sales_bill_date => generalhelper::getGetElement('billDate'),
                ':' . salesbill_customer_id => $customerId,
                ':' . salesbill_vat_total => generalhelper::getGetElement('cgstvalue'),
                ':' . salesbill_cst_total => generalhelper::getGetElement('igstvalue'),
                ':' . salesbill_running_total => generalhelper::getGetElement('subtotal'),
                ':' . salesbill_round_off => generalhelper::getGetElement('roundOff'),
                ':' . salesbill_sales_bill_total => generalhelper::getGetElement('grandTotal'),
                ':' . salesbill_company_ref_id => generalhelper::getSessionElement('beebooklogincompanyid'),
                ...
                ':' . salesbill_product_discount => 0,
                ':' . salesbill_address_id => $addressID,
                ':' . salesbill_vat_cst_flag => 1,
                ':' . salesbill_account_ref_id => 2,
                ':' . salesbill_total_currency => generalhelper::getGetElement('grandTotalCurrency'),
                ':' . salesbill_currency_value => generalhelper::getGetElement('currencyValueFinal'),
                ':' . salesbill_currency_ref_id => generalhelper::getGetElement('currencyId'),
                ':' . salesbill_cgst_total => generalhelper::getGetElement('cgstvalue'),
                ':' . salesbill_sgst_total => generalhelper::getGetElement('sgstvalue'),
                ':' . salesbill_igst_total => generalhelper::getGetElement('igstvalue')
            );
    ...
    public static function saveSmInvoiceItems() {
        $commit = 1;
        try {
            $linetotal = generalhelper::getGetElementArray('linetotal');
            $linecgstrate = generalhelper::getGetElementArray('linecgstrate');
            ...
            $linecontractNumber = generalhelper::getGetElementArray('linecontractNumber');
            $lineinvoicevalue = generalhelper::getGetElementArray('lineinvoicevalue');
            $linerateofcommision = generalhelper::getGetElementArray('linerateofcommision');
            $lineamountincurrency = generalhelper::getGetElementArray('lineamountincurrency');
            $linencurrencyvalue = generalhelper::getGetElementArray('linencurrencyvalue');
            ...
            $datafields = array(salesbillitem_sales_bill_ref_id, salesbillitem_sales_bill_date,
                ...
                salesbillitem_Discount, salesbillitem_quantity,
                salesbillitem_total, salesbillitem_chess_rate, salesbillitem_chess_total,
                salesbillitem_cgst_rate, salesbillitem_cgst_total, salesbillitem_sgst_rate, salesbillitem_sgst_total, salesbillitem_igst_rate, salesbillitem_igst_total,
                salesbillitem_UOM_ref_id, salesbillitem_packing_factor,
                salesbillitem_total_UOM_quantity, salesbillitem_hsn_code_ref_id,
                salesbillitem_sales_customer_ref_id, salesbillitem_company_ref_id,
                salesbillitem_account_year_ref_id, salesbillitem_sales_bill_type,
                salesbillitem_sales_bill_gst_type, salesbillitem_commodity_ref_id,
                salesbillitem_bags,
                salesbillitem_description, salesbillitem_total_invoice, salesbillitem_rateofcommission,
                salesbillitem_currency_total, salesbillitem_currency_value, salesbillitem_currency_ref_id,
                salesbillitem_contractnumber, salesbillitem_cst_rate, salesbillitem_cst_total, salesbillitem_vat_rate, salesbillitem_vat_total, salesbillitem_total_weight,
            );
            for ($increment = 0; $increment < count($linetotal); $increment++) {
                $linecgsttotal = $linecgstrate[$increment] * $linequantity[$increment] * $linerate[$increment] / 100;
                $linesgsttotal = $linesgstrate[$increment] * $linequantity[$increment] * $linerate[$increment] / 100;
                ...
                $datafieldsValue = array(salesbillitem_sales_bill_ref_id => self::$salesBillId,
                    ...
                    salesbillitem_total_invoice => $lineinvoicevalue[$increment],
                    salesbillitem_rateofcommission => $linerateofcommision[$increment],
                    salesbillitem_currency_total => $lineamountincurrency[$increment],
                    salesbillitem_currency_value => $linencurrencyvalue[$increment],
                    salesbillitem_currency_ref_id => generalhelper::getGetElement('currencyId'),
                    salesbillitem_contractnumber => $linecontractNumber[$increment],
                    salesbillitem_cst_rate => 0,
                    salesbillitem_cst_total => 0,
                    salesbillitem_vat_rate => 0,
                    salesbillitem_vat_total => 0,
                    salesbillitem_total_weight => 0,
                );
                $insert_values = array_merge($insert_values, array_values($datafieldsValue));
                $question_marks[] = '(' . generalhelper::placeholders('?', sizeof($datafields)) . ')';
            }
    ...
    public static function saveSmStock() {
        $commit = 1;
        try {
            $start = self::$salesBillItemLastId;
            $end = $start + self::$salesBillItemCount;
    ...
    public static function saveSmDayTransaction() {
        $commit = 1;
        try {
            $insert_values = array();
            $billType = generalhelper::getGetElement('billType');
    ...
    public static function saveSmCustomerTransaction() {
        $commit = 1;
        try {
            $insert_values = array();
            $datafields = array(customer_transaction_date, customer_transaction_customer_ref_id,
    ...
    public static function saveSmAccountTransaction() {
        $commit = 1;
        try {
            $insert_values = array();
            $datafields = array(account_transaction_date, account_transaction_type,
    ...
    public static function smUpdateInvoice() {
        self::$db->beginTransaction();
        $commit = 1;
        $billType = generalhelper::getGetElement('billType');
        $commit = self::removeBill();
